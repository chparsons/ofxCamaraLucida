...
namespace cml
{
    Kinect::Kinect(const OpticalDevice::Config& config, int xoff) 
    ...
    void Kinect::unproject( 
            int x2d, int y2d, 
            float z, float *x, float *y)
    {
        *x = (x2d + xoff - _cx) * z / _fx;
        *y = (y2d - _cy) * z / _fy;
    };

    void Kinect::unproject(int x2d, int y2d, float z, ofVec2f *p2)
    {
        unproject( x2d, y2d, z, &(p2->x), &(p2->y) );
    };

    ofVec2f Kinect::unproject(int x2d, int y2d, float z)
    {
        float x, y;
    ...
    void Kinect::project(const ofVec3f& p3, float *x, float *y)
    {
        *x = (p3.x * _fx / p3.z) + _cx - xoff;
        *y = (p3.y * _fy / p3.z) + _cy;
    };

    ofVec2f Kinect::project(const ofVec3f& p3)
    {
        float x, y;
        project( p3, &x, &y );
        return ofVec2f( x, y );;
    ...
    float Kinect::z_mts(uint16_t *raw_depth_pix, int _x, int _y)
    {
    ...
    void Kinect::change_xoff( int val )
    {
        xoff += val;
    };
